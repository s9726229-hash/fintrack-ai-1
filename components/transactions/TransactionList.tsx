...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete, onEdit }) => {
  ...
  const getIcon = (type: Transaction['type']) => {
      switch(type) {
          case 'INCOME':
              return <div className="p-2.5 rounded-full shrink-0 bg-[#FBEAEA] text-[#C4523A]"><ArrowUpCircle size={18}/></div>;
          case 'EXPENSE':
              return <div className="p-2.5 rounded-full shrink-0 bg-[#EAF1EC] text-[#6B9080]"><ArrowDownCircle size={18}/></div>;
          case 'DIVIDEND':
              return <div className="p-2.5 rounded-full shrink-0 bg-amber-500/10 text-amber-600"><Coins size={18}/></div>;
          default:
              return null;
      }
  };
  ...
  return (
    <div className="max-h-[600px] flex flex-col">
      ...
      <div className="overflow-y-auto flex-1 space-y-2 px-1 pt-1">
          {transactions.map(t => (
          <div key={t.id} className="bg-white border border-[#EDE4D6] p-3 rounded-xl flex items-center group hover:border-[#C4A98A] transition-all">
              <div className="flex-1 flex items-center gap-3 min-w-0">
                  {getIcon(t.type)}
                  <div className="min-w-0">
                      <div className="font-bold text-[#3D3428] text-sm flex items-center gap-1.5 truncate">
                          {t.item}
                          {t.invoiceId && <CheckCircle2 size={10} className="text-[#C4523A]"/>}
                      </div>
                      <div className="text-[10px] text-[#A69B87] flex gap-1.5 items-center">
                          <span className="tabular-nums">{t.date}</span>
                          <span className="w-0.5 h-0.5 rounded-full bg-[#C4A98A]"></span>
              ...
              <div className="w-28 flex-shrink-0 text-right pr-4">
                  <span className={`text-base font-bold whitespace-nowrap tabular-nums ${getAmountStyle(t.type)}`}>
                    {formatAmount(t)}
                  </span>
              </div>
              ...
              <div className="w-[88px] flex-shrink-0">
                  <div className="flex items-center justify-center gap-2">
                    <button onClick={() => onEdit(t)} className="p-2 rounded-lg text-[#A69B87] hover:bg-[#F6E4DE] hover:text-[#C4523A] transition-all" title="編輯">
                        <Pencil size={18}/>
                    </button>
                    <button onClick={() => onDelete(t.id)} className="p-2 rounded-lg text-[#A69B87] hover:bg-[#F6E4DE] hover:text-[#B45B45] transition-all" title="刪除">
                        <Trash2 size={18}/>
                    </button>
                  </div>
              </div>
