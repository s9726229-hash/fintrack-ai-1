...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete, onEdit }) => {
  ...
  const getIcon = (type: Transaction['type']) => {
      switch(type) {
          case 'INCOME':
              return <div className="p-2 rounded-full shrink-0 bg-[#FBEAEA] text-[#C4523A]"><ArrowUpCircle size={16}/></div>;
          case 'EXPENSE':
              return <div className="p-2 rounded-full shrink-0 bg-[#EAF1EC] text-[#6B9080]"><ArrowDownCircle size={16}/></div>;
          case 'DIVIDEND':
              return <div className="p-2 rounded-full shrink-0 bg-amber-500/10 text-amber-600"><Coins size={16}/></div>;
          default:
              return null;
      }
  };
  ...
  return (
    <div className="max-h-[600px] flex flex-col">
      ...
      <div className="sticky top-[45px] z-10 flex items-center bg-[#FBF7F0]/95 backdrop-blur-sm px-3 py-2 border-b border-[#EDE4D6] text-xs text-[#A69B87] uppercase tracking-wider font-medium">
          <div className="flex-1 pl-10">項目</div>
      ...
      <div className="overflow-y-auto flex-1 space-y-2 px-1 pt-1">
          ...
          <div key={t.id} className="bg-white border border-[#EDE4D6] p-3 rounded-xl flex items-center group hover:border-[#C4A98A] transition-all">
              ...
              <div className="w-24 flex-shrink-0 text-right pr-2">
                  <span className={`text-sm font-bold whitespace-nowrap tabular-nums ${getAmountStyle(t.type)}`}>
                    {formatAmount(t)}
                  </span>
              </div>
